...
public struct MultiCache: Cache {

	// MARK: - Properties

	public let caches: [AnyCache]

	// MARK: - Initializers

	public init(caches: [AnyCache]) {
		self.caches = caches
	}
	...
	// MARK: - Private
	private func coordinate(block: ((AnyCache, (() -> Void)?) -> Void), completion: (() -> Void)?) {
	...
	private func get(key: String, cacheIndex index: Int, completion: @escaping ((Box?) -> Void)) {
