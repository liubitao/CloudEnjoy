//
//  LSOrderFilterViewController.swift
//  CloudEnjoy
//
...
class LSOrderFilterViewController: LSBaseViewController {
    
    @IBOutlet weak var pickerView: PickerView!
    
    typealias SelectedClosure = (LSOrderServerStatus) -> Void
    var selectedClosure: SelectedClosure?
    
    var orderServerStatus: LSOrderServerStatus = .none
    
    var dataSource = ["全部", "待服务", "服务中", "已服务", "已退单"]
    
    class func creaeFromStoryboard() -> Self {
        let sb = UIStoryboard.init(name: "AlertStoryboard", bundle: nil)
        let vc = sb.instantiateViewController(withIdentifier: "LSOrderFilterViewController")
        return vc as! Self
    }
    ...
    override func setupViews() {
        ...
        let row = LSOrderServerStatus.allCases.firstIndex(of: self.orderServerStatus) ?? 0
    ...
    @IBAction func confirmAction(_ sender: Any) {
        self.orderServerStatus = LSOrderServerStatus.allCases[self.pickerView.currentSelectedIndex]
        self.selectedClosure?(self.orderServerStatus)
        self.dismiss(animated: true)
    }
    
}

extension LSOrderFilterViewController: PickerViewDataSource, PickerViewDelegate {
