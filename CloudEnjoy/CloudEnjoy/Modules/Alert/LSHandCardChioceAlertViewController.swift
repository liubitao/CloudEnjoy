...
class LSHandCardChioceAlertViewController: LSBaseViewController {
    
    @IBOutlet weak var pickerView: PickerView!
    
    
    typealias SelectedClosure = (LSHandCardModel) -> Void
    var selectedClosure: SelectedClosure?
    
    var handCardModel: LSHandCardModel = LSHandCardModel()
    var list: [LSHandCardModel] = []
    
    class func creaeFromStoryboard(handCardModel: LSHandCardModel?) -> Self {
        let sb = UIStoryboard.init(name: "AlertStoryboard", bundle: nil)
        let vc = sb.instantiateViewController(withIdentifier: "LSHandCardChioceAlertViewController") as! Self
        vc.handCardModel = handCardModel ?? LSHandCardModel()
        return vc
    }
    ...
    override func setupData() {
        Toast.showHUD()
        LSWorkbenchServer.getHandcardinfoList(cond: "", handcardtypeid: "").subscribe { listModel in
            guard let list = listModel?.list else {
                return
            }
            self.list = list
            let row = self.list.firstIndex(where: { $0.handcardid == self.handCardModel.handcardid }) ?? 0
    ...
    @IBAction func confirmAction(_ sender: Any) {
        ...
        self.handCardModel = self.list[self.pickerView.currentSelectedIndex]
        self.selectedClosure?(self.handCardModel)
        self.dismiss(animated: true)
    }
    
}

extension LSHandCardChioceAlertViewController: PickerViewDataSource, PickerViewDelegate {
    func pickerViewNumberOfRows(_ pickerView: PickerView) -> Int {
        return list.count
    }
    
    func pickerView(_ pickerView: PickerView, titleForRow row: Int) -> String {
        return list[row].handcardno
    }
