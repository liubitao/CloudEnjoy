...
#define kLabelYSpacing  25  //y轴上下间距
...
- (void)setAlpha:(CGFloat)alpha {
    ...
    [self.scrollView.subviews enumerateObjectsUsingBlock:^(UIView *subView, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([NSStringFromClass([subView class]) isEqualToString:@"UITableViewWrapperView"]) {
            NSInteger kyoDataTipsViewIndex = [self.scrollView.subviews indexOfObject:self];
            if (self.scrollView.subviews.count > idx + 1) {
                [self.scrollView exchangeSubviewAtIndex:kyoDataTipsViewIndex withSubviewAtIndex:idx + 1];
            }
            *stop = YES;
        };
    }];
    ...
    if (kyoDataTipsModel) {
        ...
        if (kyoDataTipsModel.isShowOperationButton) {
            ...
            if (kyoDataTipsModel && [kyoDataTipsModel.buttonText isKindOfClass:[NSString class]]) {
                ...
                self.btnOperation.titleLabel.font = [UIFont systemFontOfSize:kyoDataTipsModel.buttonTextSize];
...
- (void)initialize:(UIScrollView *)scrollView {
    ...
    NSDictionary *dictViews = @{@"subView" : btnBG};
    NSDictionary *dictMetrics = @{@"sep" : @(0)};
    ...
    [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-(==sep)-[subView]-(==sep)-|" options:0 metrics:dictMetrics views:dictViews]]; //上高下
    self.btnBG = btnBG;
    ...
    lblTips.textColor = [UIColor colorWithRed:((float)((0x7E & 0xFF0000) >> 16))/255.0 \
                                        green:((float)((0x7E & 0xFF00) >> 8))/255.0 blue:((float)(0x7E & 0xFF))/255.0 alpha:1.0];
    ...
    dictViews = @{@"subView" : lblTips};
    dictMetrics = @{@"sep" : @(kLabelXSpacing),
                    @"priority" : @(UILayoutPriorityDefaultHigh),
                    };
    [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-(==sep@priority)-[subView]-(==sep@priority)-|" options:0 metrics:dictMetrics views:dictViews]]; //左宽右
...
- (BOOL)isShow {
    return self.hidden == NO && self.alpha != 0;
}
