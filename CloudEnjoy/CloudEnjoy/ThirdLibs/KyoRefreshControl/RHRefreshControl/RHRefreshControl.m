...
- (void)refreshScrollViewDidScroll:(UIScrollView *)scrollView {
  ...
  if (self.state == RHRefreshStateLoading) {
		...
		offset = MIN(offset-_tableViewDefaultInsets.top, self.maximumForPull);
...
//	if (scrollView.contentOffset.y <= -(self.maximumForPull + self.minimumForStart) && !_loading) {
    if (self.state == RHRefreshStatePulling && !_loading) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            scrollView.contentInset = UIEdgeInsetsMake((self.maximumForPull + self.minimumForStart)+self->_tableViewDefaultInsets.top, self.scrollView.contentInset.left, self.scrollView.contentInset.bottom, self.scrollView.contentInset.right);
        }];
	}
}
...
- (void)refreshScrollViewDataSourceDidFinishedLoading {
    [UIView animateWithDuration:0.3 animations:^{
        [self.scrollView setContentInset:UIEdgeInsetsMake(0.0f+self->_tableViewDefaultInsets.top, self.scrollView.contentInset.left, self.scrollView.contentInset.bottom, self.scrollView.contentInset.right)];
    }];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    if ([keyPath isEqualToString:@"bounds"]) {
        if (self.refreshView.frame.size.width != self.scrollView.bounds.size.width) {
            self.refreshView.frame = CGRectMake(0, -self.maximumForPull, self.scrollView.bounds.size.width, self.maximumForPull);
            if (self.refreshDisplayType == KyoRefreshDisplayTypeDefault) {
...
#pragma clang diagnostic pop
        ...
        } else if ( self.scrollView.contentInset.top == _tableViewDefaultInsets.top + (44 + [UIApplication sharedApplication].statusBarFrame.size.height) && self.scrollView.contentInset.top > 0) { //top是64+。。时触发
            self.scrollView.contentInset = UIEdgeInsetsMake(_tableViewDefaultInsets.top, self.scrollView.contentInset.left, self.scrollView.contentInset.bottom, self.scrollView.contentInset.right);
            self.scrollView.contentOffset = CGPointMake(self.scrollView.contentOffset.x, self.scrollView.contentOffset.y + (44 + [UIApplication sharedApplication].statusBarFrame.size.height));
        }
    }
}
