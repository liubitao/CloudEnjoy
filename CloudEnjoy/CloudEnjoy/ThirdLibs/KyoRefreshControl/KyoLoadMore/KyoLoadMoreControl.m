...
- (id)initWithScrollView:(UIScrollView *)scrollView withIsCanShowNoMore:(BOOL)isCanShowNoMore {
    ...
    if (self) {
        ...
        [self resetInset:self.scrollView.contentInset];
...
- (BOOL)isLoadMore {
    if (self.state == KyoLoadMoreControlStateNone) {
        return NO;
    }
...
//重置inset
- (void)resetInset:(UIEdgeInsets)inset {
    if (_isCanShowNoMore) {
        self.defaultInsets = UIEdgeInsetsMake(inset.top, inset.left, inset.bottom + 30, inset.right);
    } else {
        self.defaultInsets = inset;
    }
    self.newInsets = UIEdgeInsetsMake(inset.top, inset.left, inset.bottom + 30, inset.right);
